...
public class Model {
	 ...
	 * or in the same position as another plant. Plants are generated in this way
	 * until the list reaches size numPlants.
	 * <p>
	 * The "fish" ArrayList is instantiated. Now randomly placed Fish objects are
	 * put into the List. Their directions are also randomly selected. The positions
	 * are chosen so that they are never above rocks, plants, or other fish. Fish
	 * are generated in this way until the list reaches size numFish.
	 * 
	 ...
	 * THIS METHOD HAS BEEN WRITTEN FOR YOU.
	 * <p>
	 * When a plant gets bigger than Plant.MAX_PLANT_SIZE, it will explode into 2 to
	 * 9 smaller plants, whose sizes add up to the size of the original plant. The
	 * smaller plants will be placed in the 9 regions of the landscape array that
	 * surround the original plant. If there are rocks, fish, or other plants
	 * already occupying these adjacent regions, then fewer than 9 plants are
	 * created. If there are no available regions nearby, the plant will not
	 * explode.
	 */
	public void plantExplosions() {
		...
		while (i.hasNext()) {
			...
			if (curr.getSize() > Plant.MAX_PLANT_SIZE) {
				...
				/*
				 * locations of 8 little plants are determined by these offsets to the
				 * coordinates of the plant that is exploding.
				 */
				...
				if (count > 0) {
					...
					/*
					 * Since we've modified the List, the original iterator is no longer useful.
	 ...
	 * THIS METHOD HAS BEEN WRITTEN FOR YOU!
	 * <p>
	 * When a fish gets bigger than Fish.MAX_FISH_SIZE, it will explode into 4 to 8
	 * smaller fish, whose sizes add up to the size of the original fish. The
	 * smaller fish will be placed in the eight regions of the landscape array
	...
	public void fishExplosions() {
		...
		while (i.hasNext()) {
			...
			if (curr.getSize() > Fish.MAX_FISH_SIZE) {
				...
				/*
				 * locations of the 8 little fish are determined by these offsets to the
				 * coordinates of the original fish that is exploding
				 */
				int[] dx = { 0, 1, 1, 1, 0, -1, -1, -1 };
				int[] dy = { -1, -1, 0, 1, 1, 1, 0, -1 };
				...
				if (count > 3) {
					...
					/*
					 * Since we have modified the List, the original Iterator is no longer valid.
					 * We'll start iterating again from the beginning.
					 */
					i = fish.iterator();
				}
			}
	...
	public boolean isSpaceAvailable(int r, int c) {
		boolean ifFish = false, ifPlant = false;
		...
		for (Plant y : plants) {
			if (y.getRow() == r && y.getCol() == c) {
				ifPlant = true;
				break;
			}
		}

		if (ifFish || ifPlant || landscape[r][c] == ROCK) {
			return false;
		} else {
			return true;
		}
	 ...
	 * Copy Constructor.
	 * <p>
	 * Since landscape is immutable, it is be copied with just a shallow copy. Fish
	 * and Plants are mutable, so they must be copied with a DEEP copy! (WARNING:
	 * Each fish and each plant must be copied.)
	 */
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		...
		if (landscape[row - 1][col] == ROCK && landscape[row + 1][col] == ROCK && landscape[row][col - 1] == ROCK
				&& landscape[row][col + 1] == ROCK) {
			return true;
		} else {
			return false;
		}
	 ...
	 * <p>
	 * 2. If this fish's direction is not equal to one of the codes UP, DOWN, LEFT,
	 * or RIGHT, then throw an IllegalFishDirectionException, passing this fish's
	 ...
	 * setRandomDirection method. Repeat this step until the fish is no longer about
	 * to hit a rock. Do not make any EXTRA calls to setRandomDirection or you will
	 * fail our tests!
	 */
	...
	/**
	 * Note: This method assumes that each live fish that is not surrounded by rocks
	 * is already facing a direction where there is no rock! (Typically the call to
	 * this method should immediately follow a call to "turnFish", which ensures
	 ...
	 * along to the next fish in the list. (This fish does not move, does not eat,
	 * does not fight.)
	 * <p>
	 ...
	 * fishEatPlant.
	 * <p>
	 * 4. Check if there is another fish (distinct from this fish) that is in the
	 * same location as this fish. If so, have the two fish fight each other by
	 * calling the fight method. IMPORTANT -- the fight method is not symmetrical.
	 * You must use THIS fish as the current object, and pass the OTHER fish as the
	...
	public void moveFish() {
		for (Fish x : fish) {
			if (x.isAlive()) {
				...
				x.move();
				for (Plant y : plants) {
					if (y.getCol() == x.getCol() && y.getRow() == x.getRow() && y.isAlive()) {
						fishEatPlant(x, y);
					}
				}
				for (int i = 0; i < fish.size(); i++) {
					if (fish.indexOf(x) != i && fish.get(i).getRow() == x.getRow()
							&& fish.get(i).getCol() == x.getCol()) {
						x.fight(fish.get(i));
					}
				}
			}
	 ...
	 * to the list. Instead throws an IllegalPlantPositionException, passing
	 * IllegalPlantPositionException.TWO_PLANTS_IN_ONE_PLACE to the constructor.
	 * <p>
	 ...
	 * <p>
	 * First checks if the landscape in the fish's location is equal to ROCK. If it
	 * is, then the fish is not added to the list. Instead, throws an
	 ...
	 * same location as the parameter. If one is found, then the fish is not added
	 * to the list. Instead throws an IllegalFishPositionException, passing
	 * IllegalFishPositionException.TWO_FISH_IN_ONE_PLACE to the constructor.
	 * <p>
