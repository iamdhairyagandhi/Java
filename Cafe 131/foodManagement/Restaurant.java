...
public class Restaurant {
	 ...
	 * @param startingCash cash amount that the restaurant will have, measured in
	 *                     pennies
	 */
	...
	public boolean checkIfInInventory(Entree entree) {
		...
		SortedListOfImmutables foodInEntree = entree.getFoodList();
		if (inventory.checkAvailability(foodInEntree)) {
			return true;
		}
		return false;
	}
	 ...
	 * wholesale cost of all of the food items combined exceeds the amount of cash
	 * on hand, then NONE of the food items are added to the inventory. If the
	 * amount of cash on hand is sufficient to pay for the shipment, then the amount
	 * of cash on hand is reduced by the wholesale cost of the shipment.
	 * 
	 ...
	 *         added because their wholesale cost exceeds the current cash on hand
	 */
	 ...
	 * If the inventory does not contain all of the items required for this Entree,
	 * then NOTHING is removed from the inventory. If the inventory contains all of
	 * the required items, then the amount of cash on hand is INCREASED by the
	 * retail value of the Entree.
	 * 
	 * @param entree Entree that has been ordered
	 * @return true if the food items are removed from the inventory; false if the
	 *         food items were not removed because one or more required items were
